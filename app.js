...
if(qrcodeEl){
  ...
  try{
    if(window.QRCode){
      ...
      qrcodeEl.innerHTML = `<p class="muted">QR no disponible. Usá la URL de la página.</p>`;
    }
  }catch(error){
...
function abrirContacto(tipo){
  ...
  if(contactTipo) contactTipo.value = tipo;
  if(contactTitle) contactTitle.textContent = tipo === 'voluntario' ? 'Anotarme como voluntario' : 'Quiero ser sponsor';
  if(contactMsg){
...
document.querySelectorAll('[data-open-contact]').forEach(btn => {
  btn.addEventListener('click', () => abrirContacto(btn.dataset.openContact || 'sponsor'));
});

document.querySelector('.dialog-close')?.addEventListener('click', cerrarContacto);
contactDialog?.addEventListener('click', (e) => {
  if(e.target === contactDialog) cerrarContacto();
});

document.addEventListener('keydown', (e) => {
  if(e.key === 'Escape' && contactDialog?.open) cerrarContacto();
});
...
contactForm?.addEventListener('submit', async (e) => {
  ...
  try{
    ...
    setTimeout(cerrarContacto, 1200);
  }catch(error){
    console.error(error);
    contactMsg.className = 'form-msg bad';
